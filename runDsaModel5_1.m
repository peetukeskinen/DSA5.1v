...
function [final_adjustment_path, nomNetPrimaryExpenditure,params,BindingCriteria] = runDsaModel5_1(params)
close all;
...
%
% NOTE: Code uses function project_debt5_1v.m to find minimum 
...
for i = 1:length(adjustmentGrid)
    [debtPaths(:,i),nominalGrowthPaths(:,i),realGrowthPaths(:,i),~,~,sbpPaths(:,i),obPaths(:,i),sbPaths(:,i),realGDPPaths(:,i)] = project_debt5_1v(scenario,adjustmentGrid(i), iir, potgdp,...
        ...
        zeros(totalPeriods,1),zeros(totalPeriods,1),adjustmentPeriods,theta_lt);
end
...
% calculate debt path with minimum adjustment (optimalAdjustment) found in the earlier section 
    [debt_path,optimal_Gn,optimal_Gr,iir_path,pb_path,spb_path,ob_path,sb_path,~] = project_debt5_1v(scenario,optimalAdjustment, iir, potgdp,...
        og, epsilon,m,dcoa,dprop,sfa,inflation,rgdp_initial, debt_initial,alpha_initial,beta_initial, spb,...
        i_st,i_lt,...
        share_lt_maturing,pb,ob,sb,deter,zeros(totalPeriods,1),...
        zeros(totalPeriods,1),zeros(totalPeriods,1),adjustmentPeriods,theta_lt);
...
pb_deficit = NaN(totalPeriods,length(adjustmentGrid)); % shell for primary deficit
    ...
    % Calculate paths for ALL values of adjustmentGrid
    for i = 1:length(adjustmentGrid)
        [D_safeguard(:,i),~,~,~,pb_deficit(:,i),~,ob_benchmark(:,i),sb_safeguard(:,i),~] = project_debt5_1v(scenario,adjustmentGrid(i), iir, potgdp,...
            og, epsilon,m,dcoa,dprop,sfa,inflation,rgdp_initial, debt_initial,alpha_initial,beta_initial, spb,...
            i_st,i_lt,...
            share_lt_maturing,pb,ob,sb,deter,zeros(totalPeriods,1),...
            zeros(totalPeriods,1),zeros(totalPeriods,1),adjustmentPeriods,theta_lt);
...
disp([adj_time, round(nomNetPrimaryExpenditure,2) ]);  % Display years alongside the adjustment path values
